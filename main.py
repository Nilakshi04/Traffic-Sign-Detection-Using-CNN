...
from convolutionalNeuralNetwork import training, getLabel
...
def localization(image, min_size_components, similitary_contour_with_circle, model, count, current_sign_type):
    ...
    if sign is not None:
        ...
        sign_type = sign_type if sign_type <= 10 else 10
...
def main(args):
    ...
    while True:
        ...
        print("Sign:{}".format(SIGNS[sign_type]))
        if prevSign!=SIGNS[sign_type] and 'ERROR'!=SIGNS[sign_type] and 'OTHER'!=SIGNS[sign_type]:


                myobj = gTTS(text='Sign detected is'+str(SIGNS[sign_type]), lang=language, slow=False) 

                    # Saving the converted audio in a mp3 file named 
                    # welcome
                i=i+1
                myobj.save("welcome"+str(i)+'.mp3')
                mixer.music.load("welcome"+str(i)+'.mp3') 
                mixer.music.queue("welcome"+str(i)+'.mp3') 
                mixer.music.play()   
                prevSign=SIGNS[sign_type]           
        ...
        if sign_type > 0 and (not current_sign or sign_type != current_sign):
            ...
            position = [count, sign_type if sign_type <= 10 else 10, coordinate[0][0], coordinate[0][1], coordinate[1][0], coordinate[1][1]]
            ...
            if sign_type > 0:
                ...
                position = [count, sign_type if sign_type <= 10 else 10, left, top, right, bottom]
                cv2.rectangle(image, coordinate[0],coordinate[1], (0, 255, 0), 1)
                font = cv2.FONT_HERSHEY_PLAIN
                cv2.putText(image,text,(coordinate[0][0], coordinate[0][1] -15), font, 1,(0,0,255),2,cv2.LINE_4)
            elif current_sign:
                position = [count, sign_type if sign_type <= 10 else 10, tl[0], tl[1], br[0], br[1]]
